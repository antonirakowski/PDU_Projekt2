...
---
```{r Importing the necessary libraries, warni}
...
types <- names(count_list_sorted)[1:20]
counts <- unlist(count_list_sorted)[1:20]
# Create a bar plot
...
ggplot(as.data.frame(types, counts), aes(x = reorder(types, -counts), y = counts, fill = counts > mean(counts))) +
  ...
  labs(title = "Counts of Crime Types", x = "Crime Types", y = "Count") +
...
print(unique(unlist(crimes %>% filter(offense_category == crimeType) %>% select(offense_description))))


crimes <- crimes %>%
  mutate(Description = case_when(
    Primary.Type == crimeType & str_detect(Description, "POSS") ~ "POSSESS",
    Primary.Type == crimeType & ( str_detect(Description, "DELIVER") | 
         str_detect(Description, "MANU")) ~ "MANU/DELIVER",
    TRUE ~ Description
  ))
...
plot(df_Possess$mean_temp, df_Possess$count, main = title, 
     type="p", pch=16, col=rgb(1,0,0,1))
#lines(df_Possess$mean_temp, df_ManuDeliver$count, main = title, 
#     type="p", pch=16, col=rgb(0,0,1,1))
#lines(df_Possess$mean_temp, df_Other$count, main = title, 
#    type="p", pch=16, col=rgb(0,1,0,1))
...
for (i in colnames(df_Homicide_normalized)[2:4]){
  ...
  p<-ggplot(df_Homicide_normalized, aes(x = .data[[i]], y = count)) +
  geom_point(color = rgb(0,0,0,0.5), shape = 16) +
  labs(title = title, x = "", y = "") +
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 1)) # Adjust axis text angle if needed
  print(p)
