...
We can see that duplicate datetime values have been created due to the 
rounding of minutes. Let's take the mean values of each hour
...
Let's study the correlations between the power consumption values 
...
Conclusion 1: General diffuse flows seem to be influenced by the 
evapotranspiration, vapour pressure deficits,temperature and relative humidity

Conclusion 2: Diffuse flows seem to be influenced by the evapotranspiration rate 
and vapour pressure deficits
...
Creating new features to find new correlations and strengthen the existing ones 
(that is: VPD, TEMP, HUM)
...
```{r}
correlated_data2 %>% mutate(GDF = stand(GDF), ET = stand(ET), 
GDF_prev = stand(GDF_prev), DF_prev = stand(DF_prev)) -> stand_data
```
...
```{r}
plot11 <- ggplot(stand_data, aes(x = ET, y = GDF)) + 
geom_point(size = 0.5, color = "blue") + 
labs(title = "y = GDF(ET)", x ="ET", y = "GDF") + 
theme_minimal()
print(plot11)
...
```{r}
plot22 <- ggplot(stand_data, aes(x = GDF_prev, y = GDF)) + 
geom_point(size = 0.5, color = "blue") + 
labs(title = "y = GDF(GDF_prev)", x ="GDF_prev", y = "GDF") + 
theme_minimal()
print(plot22)
...
```{r}
plot33 <- ggplot(stand_data, aes(x = DF_prev, y = GDF)) + 
geom_point(size = 0.5, color = "blue") + 
labs(title = "y = GDF(DF_prev)", x ="DF_prev", y = "GDF") + 
theme_minimal()
print(plot33)
...
The data is clustered, therefore we will try the following: 
random forest regressor, gradient boosting, k-nearest neighbours regressor 
...
```{r}
stand_data %>% mutate(GDF = format(GDF, scientific = FALSE), 
ET = format(ET, scientific = FALSE), 
GDF_prev = format(GDF_prev, scientific = FALSE), 
DF_prev = format(DF_prev, scientific = FALSE)) -> data_to_save2
```
...
```{r}
write.csv(data_to_save, "NormData.csv", row.names = FALSE)
write.csv(data_to_save2, "StandData.csv", row.names = FALSE)
```
